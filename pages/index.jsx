...
function MissingTab(){
  ...
  const runAnalyze=async(post,idx)=>{
    ...
    try{
      ...
      // ── Step 1: 제목 + 해시태그 기반 키워드 추출 ──
      const extractKeywords = (title, tags) => {
        const seen = new Set();
        const result = [];
        const add = (k) => {
          k = (k||'').trim();
          if(k && k.length >= 2 && !seen.has(k) && result.length < 15){
            seen.add(k); result.push(k);
          }
        };
        ...
        // 2) 제목에서 불용어 제거 후 의미 단어 추출
        const stopWords = new Set([
          '하는','하기','방법','방식','하면','이란','이다','입니다','합니다','했다','한다',
          '있는','없는','위한','대한','통해','으로','에서','부터','까지','그리고','또는',
          '하지만','그러나','그래서','때문','경우','정도','정말','진짜','너무','아주',
          '매우','바로','이제','같은','다른','가장','특히','주로','항상','자주','가끔',
          '전혀','완전','약간','조금','많이','적게','안된','안되','안다','안가','안고',
          '안간','되는','되다','된다','됩니','되면','이후','이전','현재','최근','모든',
          '즉시','실천한','실천','이후','적용','따른','위해','관련','통한','대비','예방',
        ]);

        // 한글+영문 모두 추출
        const words = (title.match(/[가-힣]+|[A-Za-z0-9]+/g)||[])
      ...
      const kws = extractKeywords(post.title, post.tags);
